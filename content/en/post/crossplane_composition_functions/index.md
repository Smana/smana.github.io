...
date = "2023-10-23"
summary = "Discover how the use of advanced Crossplane concepts in a concrete example could signal a key milestone for the project. Is this the change we've been waiting for?"
featured = true
codeMaxLines = 21
usePageBundles = true
toc = true
...
There you can find numerous sources that help me construct my blog articles. Feel free to provide feedback, open issues if necessary... 🙏
{{% /notice %}}
...
It leverages the Kubernetes API to define and orchestrate not just infrastructure elements like storage and networking, but also numerous other components (refer to the list of [providers](https://marketplace.upbound.io/providers)). This method provides developers with a **simplified interface**, representing an abstraction layer that conceals the more complex technical details of the underlying infrastructure.

To achieve my goal of creating an RDS database when deploying the Harbor application, I first looked for a relevant example. For this purpose, I used the [Upbound marketplace](https://marketplace.upbound.io/), where numerous _Compositions_ can be found that serve as starting points.
...
* ▶️ Creation of a [**ExternalName**](https://kubernetes.io/docs/concepts/services-networking/service/#externalname) type service with a predictable name that can be used in Harbor's configuration.
...
They simply declare a _**Claim**_ which represents the level of abstraction exposed to the users.
...
* The `master` user's password is retrieved from a `harbor-pg-masterpassword` secret, generated via an [External Secret](https://github.com/Smana/demo-cloud-native-ref/blob/main/tooling/base/harbor/externalsecret-sqlinstance-password.yaml).
* Once the instance is created, the connection details are stored in a **secret** `xplane-harbor-rds`.

This is where we can fully appreciate the **power of Crossplane Compositions**! Indeed, **numerous resources** are transparently generated, as illustrated by the following diagram:

<center><img src="rds-composition.png" width="1400" height="1400" alt=""></center>

After a few minutes, all the resources are created. (ℹ️ The [Crossplane CLI](https://docs.crossplane.io/latest/cli/) now enables many operations, including visualizing the resources of a Composition. It is named `crank` to differentiate it from the `crossplane` binary, which is the executable running on Kubernetes.)
...
And Harbor becomes accessible thanks to Cilium and Gateway API (You can take a look at a [previous post](https://blog.ogenki.io/post/cilium-gateway-api/) on the topic 😉)

<center><img src="harbor_fresh.png" width="700" height="700" alt=""></center>

{{% notice info EnvironmentConfig %}}
The [**EnvironmentConfigs**](https://docs.crossplane.io/latest/concepts/environment-configs/) enable the use of **cluster-specific variables**. These configuration elements are loaded into memory and can then be utilized in the composition.

Since the EKS cluster is created with [Opentofu](https://opentofu.org/), we store its properties using Flux variables. (more info on variable substitution [here](https://blog.ogenki.io/post/terraform-controller/#substition-de-variables))
...
These variables can then be used in _Compositions_ via the **`FromEnvironmentFieldPath`** directive. For instance, to allow pods to access our RDS instance, we authorize the VPC's CIDR:
...
⚠️ As of the time of writing this article, the feature is still in alpha.
{{% /notice %}}


## 🛠️ Composition Functions

Composition Functions (**`Composition Functions`**) represent a significant advancement in the development of Compositions. Traditionally, _patching_ within a composition had certain **limitations**, such as the inability to use conditions, loops in the code, or to execute advanced functions (e.g., subnet calculations, checking the status of external resources).

_Composition Functions_ overcome these limitations and are essentially programs that extend the templating capabilities of resources within Crossplane. They can be written in **any programming language**, thus offering immense flexibility and power in defining compositions. This allows for complex tasks such as conditional transformations, iterations, and dynamic operations.
...
But **let's get back to our RDS composition 🔍!** It indeed uses this new way of defining `Compositions` and consists of three stages:
...
1. The syntax of the first stage, `patch-and-transform`, might look familiar 😉. It is indeed the traditional **patching** method of Crossplane, but this time executed as a function in the Pipeline.
2. The second stage involves calling the [**function-go-templating**](https://github.com/crossplane-contrib/function-go-templating) function, which we will discuss in more detail shortly.
3. Finally, the last stage uses the [function-auto-ready](https://github.com/crossplane-contrib/function-auto-ready) function, which checks whether the composite resource (`XR`) is ready. This means that all the resources composing it have reached the `Ready` state.
...
As mentioned earlier, the power of _Composition Functions_ lies primarily in the fact that any programming language can be used. It's notably possible to generate resources from Go templates. **This isn't so different from writing [Helm](https://helm.sh/) Charts**.
...
Then it's your turn to play! For example, there is little difference in generating a MariaDB or PostgreSQL database, so we can formulate conditions like the following:
...
This also allowed me to define a list of databases along with the associated owner.
...
When we talk about Infrastructure As Code, Terraform often comes to mind first. This tool, supported by a vast community and boasting a well-established ecosystem, remains a top choice. However, it's interesting to ponder how Terraform has evolved in response to the new paradigms introduced by Kubernetes. We touched on this in our article on [terraform controller](https://blog.ogenki.io/post/terraform-controller/). Since then, you may have noticed the stir caused by [Hashicorp's decision to adopt the Business Source License](https://www.hashicorp.com/blog/hashicorp-adopts-business-source-license). This development sparked many reactions and might have influenced the strategy and roadmap of other solutions...

It's hard to say if it's a direct reaction, but recently, `Crossplane` updated its [charter](https://blog.crossplane.io/charter-expansion-upjet-donation/) to expand its scope to the entire ecosystem (providers, functions), notably by integrating the [**Upjet**](https://github.com/crossplane/upjet) project under the CNCF umbrella. The goal of this move is to strengthen the governance of associated projects and ultimately improve the **developer experience**.
...
❓ So, what to make of this new experience with Crossplane?

It must be said, _**Composition Functions**_ offer broad prospects, and we can expect to see many functions emerge in 2024 🚀

However, in my view, it is crucial that **development and operation tools** continue to improve to foster adoption of the project. For instance, a web interface or a k9s plugin would be useful.

For a beginner looking to develop a composition or a function, the **first step might seem daunting**. Validating a composition is not straightforward, and there aren't many examples to follow. We hope the [marketplace](https://marketplace.upbound.io/) will grow over time.

That said, these concerns have been **addressed by the Crossplane community**, especially by the SIG Dev XP, whose efforts deserve applause and who are currently doing significant work. 👏

I encourage you to closely follow the project's evolution in the coming months 👀, and to try out Crossplane for yourself to form your own opinion. For my part, I'm particularly interested in the CUElang function, a language I plan to explore soon.
